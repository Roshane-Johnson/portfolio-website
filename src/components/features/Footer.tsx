...
const data = () => ({
	navigation: {
		...
		projects: [{ name: 'Linkedin AI Profile Analyzer (In Progress)', href: '#!profile-analyzer' }],
	},
	socialLinks: [
		{ icon: Twitter, label: 'Twitter', href: 'https://twitter.com/kendrip_' },
		{ icon: Github, label: 'GitHub', href: 'https://github.com/roshane-johnson' },
		{ icon: Linkedin, label: 'LinkedIn', href: 'https://linkedin.com/in/roshane-johnson' },
	],
	bottomLinks: [{ href: '/cookie-policy', label: 'Cookie Policy' }],
})
			...
			<div className="relative w-full px-5">
				...
				<div className="container m-auto grid grid-cols-1 gap-12 py-12 md:grid-cols-2 lg:grid-cols-5">
					...
					<div className="space-y-6 lg:col-span-2">
						<Link href="/" className="inline-flex items-center gap-3">
							<span className="text-xl font-semibold">Roshane.</span>
						</Link>
						<p className="text-muted-foreground max-w-md">Building innovative solutions for modern businesses. Fast, reliable, and scalable.</p>
						...
						<form onSubmit={(e) => e.preventDefault()} className="w-full max-w-md space-y-3">
							<label htmlFor="email" className="block text-sm font-medium">
								Connect with a creative solution oriented engineer.
							</label>
							<div className="relative w-full">
								<Input type="email" id="email" placeholder="Enter your email" className="h-12 w-full" required />
								<Button type="submit" className="absolute top-1.5 right-1.5 cursor-pointer transition-all duration-1000 hover:px-10">
									Subscribe
								</Button>
							</div>
					...
					<div className="grid w-full grid-cols-2 items-start justify-between gap-8 px-5 lg:col-span-3">
						{(['company', 'projects'] as const).map((section) => (
							<div key={section} className="w-full">
								<h3 className="border-primary mb-4 -ml-5 border-l-2 pl-5 text-sm font-semibold tracking-wider uppercase">{section.charAt(0).toUpperCase() + section.slice(1)}</h3>
				...
				<div className="text-muted-foreground container m-auto flex flex-col items-center justify-between gap-4 p-4 text-xs md:flex-row md:px-0 md:text-sm">
					...
					<div className="flex items-center gap-4">
						{data().bottomLinks.map(({ href, label }, idx) => (
							<Link key={idx} href={href} className="hover:text-foreground">
								{label}
							</Link>
						))}
					</div>
